    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link to="/fpo/dashboard" className="flex items-center space-x-2.5 sm:space-x-3 shrink-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-600 text-white font-bold flex items-center justify-center text-base sm:text-lg shadow-xs shrink-0">
              KS
            </div>
            <div>
              <span className="font-bold text-slate-900 text-base sm:text-lg leading-tight block">KisanSetu</span>
              <span className="text-[10px] text-blue-700 font-semibold uppercase tracking-wider block">FPO Portal</span>
            </div>
          </Link>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-200 bg-white px-4 py-3 space-y-1 shadow-lg animate-in fade-in slide-in-from-top-2 duration-150">
            <div className="pb-2 mb-2 border-b border-slate-100 px-1">
              <div className="text-xs font-bold text-slate-800">{user?.name || 'FPO Account'}</div>
              <div className="text-[11px] text-slate-500 truncate">{user?.email}</div>
            </div>
            <Link
              to="/fpo/dashboard"
              onClick={() => setMobileMenuOpen(false)}
              className="block text-xs font-semibold text-blue-800 bg-blue-50 px-3 py-2.5 rounded-lg border border-blue-200"
            >
              Dashboard
            </Link>
            <Link
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col justify-between">
                ...
                <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-slate-100">
                  <Link
                    to={`/marketplace/analytics?district=${profile?.location?.district || 'Nashik'}${profile?.majorCrops?.[0] ? `&crop=${profile.majorCrops[0]}` : ''}`}
                    className="block text-center text-xs font-bold text-blue-800 bg-blue-50 hover:bg-blue-100 border border-blue-300 px-3 py-2 rounded-lg transition-colors"
                  >
                    📈 Analytics & Forecast →
                  </Link>
                  <Link
